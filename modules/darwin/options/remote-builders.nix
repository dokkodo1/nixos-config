...
in
{
  options.control.remoteBuilders = {
    ...
    # Darwin-specific: Linux builder VM
    enableLinuxBuilder = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = ''
        Enable NixOS Linux builder VM (Darwin only).
        Creates a lightweight headless Linux VM that automatically handles
        x86_64-linux builds on Darwin hosts. Fully automated - no manual intervention.
      '';
    };
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
    # Common configuration for using remote builders
    {
    ...
    # Darwin-specific: Linux builder VM configuration
    (lib.mkIf cfg.enableLinuxBuilder {
      nix.linux-builder = {
        ...
        # Automatically registers as a builder for x86_64-linux
    ...
    (lib.mkIf cfg.serveAsBuilder {
      ...
      users.users.${cfg.builderUser} = {
        # On Darwin, user creation is different
        # TODO: Configure when setting up the first Darwin builder
