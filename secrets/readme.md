...
- **`validateSopsFiles = false`** is set in `modules/nixos/sops.nix`. This means the build will not fail if a secret cannot be decrypted — but the secret simply will not appear in `/run/secrets/` at runtime. If a service needs it, it will fail silently or crash.
